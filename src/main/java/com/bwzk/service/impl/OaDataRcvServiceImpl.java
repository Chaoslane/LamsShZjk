...
public class OaDataRcvServiceImpl extends BaseService implements OaDataRcvService{
	...
	public void dataReceive() {
			...
			for (Map<String, Object> dataMap : dataList) {
				if (dataMap.size() > 0 && dataMap != null) {
					try {
						...
						if(qzh.equals("D0603")){
							libcode = tmWsCode;
							tableName = "d_file"+tmWsCode+"";
							if(wjlx.equals(oaWjbs)){
								dfileMapping = tmDfileSwMapping;
							}else{
								dfileMapping = tmDfileFwMapping;
							}
						}else{
							if(wjlx.equals(oaWjbs)){
						...
						for (String OAData : dataMap.keySet()) {
							...
							if (StringUtils.isNotEmpty(DaData)) {
								...
								fDtable = CommonUtil.getFDtable(fDTableList, DaData);
						...
						String InsertSql = "insert into "+tableName+"" + " (" + fields.toString()
		 ...
		 * 电子文件集成
		 */
		private void insertEfile(String oaid , String efileSql , String libcode , Integer pid){
				Integer maxDid = -1;
